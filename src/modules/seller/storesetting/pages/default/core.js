...
const Core = (props) => {
    ...
    const { data: widthData, loading: widthLoading } = themeService.getStoreConfig({
        path: 'system/media_gallery_renditions/width',
    });
    const { data: heightData, loading: heightLoading } = themeService.getStoreConfig({
        path: 'system/media_gallery_renditions/height',
    });

    React.useEffect(() => {
        BackdropLoad(loading || loadCurrency || aclCheckLoading || loadBanner || sizeLoading || widthLoading || heightLoading);
    }, [loading, loadCurrency, aclCheckLoading, loadBanner, sizeLoading, widthLoading, heightLoading]);

    if (loading || loadCurrency || aclCheckLoading || loadBanner || sizeLoading || widthLoading || heightLoading) {
    ...
    const contentProps = {
        ...
        limitWidth: widthData?.getStoreConfig,
        limitHeight: heightData?.getStoreConfig,
    };
