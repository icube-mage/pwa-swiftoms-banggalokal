...
const ContentWrapper = (props) => {
            ...
            dataShipment.forEach((shipment) => {
                ...
                if (shipment.provider !== 'FLATRATE') {
                    temp.push(['selected', Yup.array().of(Yup.number()).when('value', {
                        is: true,
                        then: Yup.array().of(Yup.number()).min(1, t('sellerwarehouse:Choose_at_least_min_key', { min: 1, key: 'service' })),
                    })]);
                }
    ...
    const handleSubmit = (input) => {
        ...
        saveSellerStore({
            ...
            window.toastMessage({
                open: true,
                text: t('sellerwarehouse:Location_has_been_saved'),
                variant: 'success',
            });
            setTimeout(() => router.push('/seller/warehouse'), 250);
        })
            .catch((e) => {
                window.backdropLoader(false);
                window.toastMessage({
    ...
    const formik = useFormik({
        ...
        validationSchema: Yup.object().shape({
            country_id: Yup.object().typeError(t('sellerwarehouse:This_is_a_Required_field'))
                .required(t('sellerwarehouse:This_is_a_Required_field')),
            region: Yup.object().typeError(t('sellerwarehouse:This_is_a_Required_field'))
                .required(t('sellerwarehouse:This_is_a_Required_field')),
            city: Yup.object().typeError(t('sellerwarehouse:This_is_a_Required_field'))
                .required(t('sellerwarehouse:This_is_a_Required_field')),
            latitude: Yup.number().typeError(t('sellerwarehouse:Value_must_be_a_number')).required(t('sellerwarehouse:This_is_a_Required_field')),
            longitude: Yup.number().typeError(t('sellerwarehouse:Value_must_be_a_number')).required(t('sellerwarehouse:This_is_a_Required_field')),
            name: Yup.string().required(t('sellerwarehouse:This_is_a_Required_field')),
            postcode: Yup.number().typeError(t('sellerwarehouse:Value_must_be_a_number')).required(t('sellerwarehouse:This_is_a_Required_field')),
            street: Yup.string().required(t('sellerwarehouse:This_is_a_Required_field')),
            telephone: Yup.string().required(t('sellerwarehouse:This_is_a_Required_field')).typeError(t('sellerwarehouse:This_is_a_Required_field'))
                .matches(useRegexPhone, t('sellerwarehouse:Invalid_phone_number_format')),
            shipping: dataShipping && Yup.array().of(Yup.string()).min(1, t('sellerwarehouse:Choose_at_least_min_key', { min: 1, key: 'provider' })),
            is_flatrate: Yup.boolean(),
            flatrate_price: !!flatRate && Yup.number().when('is_flatrate', {
                is: true,
                then: Yup.number()
                    .nullable()
                    .typeError(t('sellerwarehouse:Value_must_be_a_number')),
            }),
            ...shipmentGroup().valid,
        }),
        onSubmit: (values) => {
            ...
            if (isEdit) {
                ...
                const dataLayer = {
                    event: 'edit_location',
                    eventLabel: 'Store - Edit Location',
                };
                sendDataLayer(dataLayer);
            } else {
                const dataLayer = {
                    event: 'add_location',
                    eventLabel: 'Store - Add Location',
                };
                sendDataLayer(dataLayer);
            }
...
const Core = (props) => {
    ...
    const { loading: loadShipping, data: dataShipping } = themeService.getStoreConfig({
    ...
    React.useEffect(() => {
        BackdropLoad(aclCheckLoading || loadMap || loadEnableMap || loadStore || loadShipment || loadShipping || loadCurrency
            || getCountriesRes.loading || getCountryRes.loading || getCityKecByRegionCodeRes.loading);
    }, [aclCheckLoading, loadMap, loadEnableMap, loadStore, loadShipment, loadShipping, loadCurrency,
        getCountriesRes.loading, getCountryRes.loading, getCityKecByRegionCodeRes.loading]);

    if (aclCheckLoading || loadMap || loadEnableMap || loadStore || loadShipment || loadShipment || loadShipping || loadCurrency
        || getCountriesRes.loading || getCountryRes.loading || getCityKecByRegionCodeRes.loading) {
        return <Layout pageConfig={pageConfig} seller hideMobileMenu />;
    }

    if ((aclCheckData && aclCheckData.isAccessAllowed) === false) {
        router.push('/seller/warehouse');
        return <Layout pageConfig={pageConfig} seller hideMobileMenu />;
    }
