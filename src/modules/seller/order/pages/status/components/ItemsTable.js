            ...
            <Table className={classes.table}>
                <TableHead>
                    <TableRow className={clsx(classes.tr, 'head')}>
                        ...
                        <TableCell className={clsx(classes.th, 'left-border')}>{t('order:Failed_Reasons')}</TableCell>
                        <TableCell className={clsx(classes.th, 'right-border')}>{t('order:Recommendation')}</TableCell>
                    </TableRow>
                </TableHead>
                <TableBody>
                    {orderItem?.filter((item) => !item.isChild).map((item) => (
                        <Fragment key={item.name}>
                            <TableRow className={clsx(classes.rowItem, (!!item.remark || item.bundle_children?.length) && 'noBorder')}>
                                ...
                                {item.bundle_children?.length
                                    ...
                                    : (
                                        <>
                                            ...
                                            <TableCell className={classes.td}>{item.sell_price}</TableCell>
                                        </>
                                    )}
                                ...
                                <TableCell className={clsx(classes.td, 'left-border td-reason')}>{item.failed[0]?.reason.reason_label || ''}</TableCell>
                                <TableCell className={clsx(classes.td, 'right-border td-reason')}>
