...
const CatalogChannelSync = (props) => {
    ...
    const { data: dataListChannel, loading: loadingListChannel } = gqlService.getSellerChannelListPushProduct();
    const [onPushSellerProduct] = gqlService.pushSellerProduct();
        ...
        if (dataListSellerChannel?.length < 1) {
            ...
            for (const listSellerChannelIndex in dataListSellerChannel) {
                ...
                const channelItem = await getActivityChannelItem({
                    variables: {
                        activity_code: `push_seller_product_${channel_id}`,
                    },
                });
    ...
    const onPushSingleChannel = async (itemChannel) => {
        try {
            await onPushSellerProduct({
                variables: {
                    channel_ids: [itemChannel.channel_id],
                },
            });
            ...
            console.log('[err] push single channel', e);
            window.toastMessage({
                open: true,
                text: e.message,
                variant: 'error',
    ...
    return (
        <div className="catalog-channel-sync">
            ...
            <div className="product-data-desc-container">
                {t('common:push_product_data_desc')}
            </div>
                    ...
                    <div className="product-data-container">
                        {channels.map((item, index) => (
                            <ItemChannel
                                ...
                                onPushSingleChannel={onPushSingleChannel}
                                sendToGTM={sendToGTM}
                            />
                        ))}
                    </div>
