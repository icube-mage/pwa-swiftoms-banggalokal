...
import useStyles from '@modules/configurationwebstore/pages/default/components/style';
...
import InputSelect from '@modules/configurationwebstore/pages/default/components/inputSelect';
import InputText from '@modules/configurationwebstore/pages/default/components/inputText';

const WebstoreContent = (props) => {
    const { formik, dataWebstore, t } = props;

    const classes = useStyles();

    return (
        <>
            <h2 className={classes.titleTop}>{t('webstoreconfiguration:Webstore')}</h2>
            <Paper className={classes.container}>
                <div className={classes.content}>
                    {dataWebstore?.map((parent, index) => (
                        <Accordion
                    ...
                    <div className={classes.formFieldButton}>
                        <Button className={classes.btn} onClick={formik.handleSubmit} variant="contained">
                            {t('webstoreconfiguration:Submit')}
                        </Button>
                    </div>
                </div>
            </Paper>
