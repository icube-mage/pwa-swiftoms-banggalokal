...
const CustomTable = (props) => {
    ...
    const renderActionCheckbox = () => (
        <Grid container className={classes.checkAllContainer} alignItems="center">
            <Grid item xs={12} sm="auto">
                {!!checkedRows.length && (
                    <div className={classes.checkedLength}>{`${checkedRows.length} ${t('common:Selected')}`}</div>
                )}
            </Grid>
            {actionsCheck?.length && checkedRows.length && count
                ? actionsCheck.map((act) => (
                    <Grid key={act.label} item>
                        <Button className={classes.btnActionCheck} onClick={act.onClick} disabled={!checkedRows.length}>
                            {act.label}
                        </Button>
                    </Grid>
                ))
                ...
                {rows.map((row, rowIndex) => (
                    ...
                    >
                        ...
                        {columns.map((column, columnIndex) => (
                            <TableCell
                                key={columnIndex}
                                className={clsx(column.hidden && 'hide', classes.alignTop, 
    ...
    return (
        <>
            ...
            <TableContainer component={Paper} className={clsx(classes.tableContainer, useTabs && !separateTab && 'joinTab')}>
                {!!checkedRows.length && !!count && renderActionCheckbox()}
                {!hideHead && renderHead()}
