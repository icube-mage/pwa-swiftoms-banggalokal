...
import {
    GoogleMap, Autocomplete, Marker, useJsApiLoader,
} from '@react-google-maps/api';
import CustomTextField from '@common_textfield';
import { useTranslation } from '@i18n';
import { ERROR } from '@theme_color';
import Alert from '@common_alert/index';
import useStyles from '@common_googlemaps_autocomplete/style';
...
const IcubeMapsAutocomplete = (props) => {
    ...
    const [mapContainerStyle, setMapContainerStyle] = useState({ ...containerStyle, ...mapStyle });
    const [libraries] = useState(['places', 'geometry']);
    const isErrorForm = required && formik && (!!(formik.touched.latitude && formik.errors.latitude) || !!(formik.touched.longitude && formik.errors.longitude));
    ...
    const mapPosition = {
        lat: setZeroIfEmpty(mapPositionProps && mapPositionProps.lat),
        lng: setZeroIfEmpty(mapPositionProps && mapPositionProps.lng),
    };
    ...
    useEffect(() => {
        if (formik !== false) {
            if (formik.values.city !== '' && formik.values.city !== undefined && formik.values.city !== null) {
                fetch(
                    `https://maps.googleapis.com/maps/api/geocode/json?address=${formik.values?.city?.label
                        ?.split(', ')
                        .join('+')}&language=id&key=${gmapKey}`,
                )
    ...
    useEffect(() => {
        if (isErrorForm) {
            setMapContainerStyle({ ...containerStyle, ...mapStyle, ...containerErrorStyle });
        } else {
            setMapContainerStyle({ ...containerStyle, ...mapStyle });
        }
    }, [isErrorForm]);
    ...
    const renderMap = () => {
        return (
            <>
                ...
                <div>
                    <Alert
                        title={t('common:Set_the_right_pin')}
                        info={t('common:drag_pin_point_to_set_location')}
                        containerClassName={classes.containerAlert}
                    />
                </div>
                <GoogleMap
                    ...
                    options={{
                        restriction: {
                            ...
                            latLngBounds: new google.maps.LatLngBounds(
                                // eslint-disable-next-line no-undef
                                new google.maps.LatLng(
                                    parseFloat(stateBounds.southwest.lat !== undefined ? stateBounds.southwest.lat : mapPosition.lat - 0.025),
                                    parseFloat(stateBounds.southwest.lng !== undefined ? stateBounds.southwest.lng : mapPosition.lng - 0.025),
                                ),
                                // eslint-disable-next-line no-undef
                                new google.maps.LatLng(
                                    parseFloat(stateBounds.northeast.lat !== undefined ? stateBounds.northeast.lat : mapPosition.lat + 0.025),
                ...
                >
                    <Marker onLoad={markerLoad} position={mapPosition} onDragEnd={(event) => handleDragEnd(event)} draggable />
                </GoogleMap>
                {isErrorForm && (
                    <div
                        style={{
                    ...
                    >
                        {t('common:Please_pinpoint_your_location')}
                    </div>
                )}
            </>
        );
