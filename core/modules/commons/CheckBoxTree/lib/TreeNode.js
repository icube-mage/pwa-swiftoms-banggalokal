...
class TreeNode extends React.PureComponent {
    ...
    renderCollapseButton() {
        ...
        return (
            <Button className="rct-collapse rct-collapse-btn" disabled={expandDisabled} title={lang.toggle} onClick={this.onExpand}>
    ...
    renderBareLabel(children) {
        ...
        return (
            <span className="rct-bare-label" title={title}>
                {clickable ? (
                    <span className="rct-node-clickable" onClick={this.onClick} onKeyPress={this.onClick} role="button" tabIndex={0}>
                        {children}
                    </span>
                ) : (
                    children
    ...
    renderCheckboxLabel(children) {
        ...
        if (clickable) {
            render.push(
                <span key={1} className="rct-node-clickable" onClick={this.onClick} onKeyPress={this.onClick} role="link" tabIndex={0}>
                    {children}
                </span>
            );
        }
