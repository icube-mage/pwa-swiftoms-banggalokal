...
class CheckboxTree extends React.Component {
    static propTypes = {
        ...
        allowParentOnlyChecked: PropTypes.bool,
    ...
    onCheck(nodeInfo) {
        const { checkModel, noCascade, onCheck, allowParentOnlyChecked } = this.props;
        const model = this.state.model.clone();
        const node = model.getNode(nodeInfo.value);
        model.toggleChecked(nodeInfo, nodeInfo.checked, checkModel, noCascade, true, allowParentOnlyChecked);
    ...
    determineShallowCheckState(node, noCascade) {
        const { allowParentOnlyChecked } = this.props;
        ...
        const conditionParentChecked = allowParentOnlyChecked ? flatNode.checked : this.isEveryChildChecked(node);

        if (conditionParentChecked) {
            return 1;
        }
    ...
    renderExpandAll() {
        ...
        return (
            <div className="rct-options">
                <Button className="rct-option rct-option-expand-all" title={lang.expandAll} onClick={this.onExpandAll}>
                    {expandAll}
                </Button>
                <Button className="rct-option rct-option-collapse-all" title={lang.collapseAll} onClick={this.onCollapseAll}>
                    {collapseAll}
                </Button>
            </div>
        );
    ...
    render() {
        const { direction, disabled, iconsClass, nodes, nativeCheckboxes } = this.props;
        const { id } = this.state;
