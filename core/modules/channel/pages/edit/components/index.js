...
const ChannelEditContent = (props) => {
    const {
        formik, queryRoute, channel, getMarketplaceDefaultShippingMethods,
        getMarketplaceDefaultShippingMethodsRes, marketplaceName, t, configSeller,
    } = props;
    ...
    return (
        <>
            <Button
                ...
                onClick={() => (queryRoute.length === 2 && !!queryRoute[1]
                    ? router.push(`/integration/${queryRoute[1]}`) : router.push('/oms/channel'))}
                variant="contained"
            ...
            <h2 className={classes.titleTop}>{t('channel:Edit_Sales_Channel')}</h2>
            <Paper className={classes.container}>
                <div className={classes.content}>
                            ...
                            <Tooltip
                                title={(
                                    <div style={{ whiteSpace: 'pre-line' }}>
                                        {t('channel:\"One_Store_Priority_Only\"_means_allocate_to_one_store_only_if_not_available_mark_order_as_Failed__Allocation_not_found')}
                                        <br />
                                        <br />
                                        {t('channel:\"Split_Shipment_if_One_Store_Not_Available\"_means_allocate_to_one_store_that_can_fulfill_all_items_if_not_available_allocate_to_more_than_one_store')}
                                        <br />
                                        <br />
                                        {t('channel:\"Split_Shipment\"_means_order_can_be_allocate_to_more_than_one_store')}
                                    </div>
                                )}
                                placement="top"
                                arrow
                    ...
                    <div className={classes.formField}>
                        <div className={classes.divLabel}>
                            <span className={classes.label}>{t('channel:Auto_Create_Product_On_Order_Allocation')}</span>
                        </div>
                        <Switch
                            id="is_auto_create_product"
                            name="is_auto_create_product"
                            value={formik.values.is_auto_create_product}
                            onChange={formik.handleChange}
                        />
                        <div className={classes.tooltip}>
                            <Tooltip
                                title={t('channel:Select_Yes_to_auto_create_unregistered_product_on_order_allocation')}
                                placement="top"
                                arrow
                                classes={{ tooltip: classes.tooltipWidth }}
                            >
                    ...
                    <div className={classes.formField}>
                        <div className={classes.divLabel}>
                            <span className={classes.label}>{t('channel:Use_Customer_Review')}</span>
                        </div>
                        <Switch
                            id="use_customer_review"
                            name="use_customer_review"
                            value={formik.values.use_customer_review}
                            onChange={formik.handleChange}
                        />
                        <div className={classes.tooltip}>
                            <Tooltip
                                title={t('channel:This_setting_is_one_of_the_parameters_that_can_changes_shipment_status_to_\"Delivered\"_Set_Yes_to_mark_shipment_as_Delivered_after_the_Customer_Review')}
                                placement="top"
                                arrow
                                classes={{ tooltip: classes.tooltipWidth }}
                            >
                    ...
                    {configSeller
                        && (
                            <div className={classes.formField}>
                                <div className={classes.divLabel}>
                                    <span className={classes.label}>{t('channel:Multiseller_Channel')}</span>
                                </div>
                                <Switch
                                    id="is_multiseller"
                                    name="is_multiseller"
                                    value={formik.values.is_multiseller}
                                    onChange={formik.handleChange}
                                />
                                <div className={classes.tooltip}>
                                    <Tooltip
                                        title={t("channel:Seller's_store_location_will_be_assigned_to_Multiseller_Channel_automatically")}
                                        placement="top"
                                        arrow
                                        classes={{ tooltip: classes.tooltipWidth }}
                                    >
                ...
                {channel.framework === 'Marketplace'
                    ? (
                        <div className={classes.content}>
                            <h2 className={classes.title}>{t('channel:Marketplace_Credentials')}</h2>
                            {channel?.credentials?.map((cred) => (

                                cred.type === 'default_shipping_method'
                                    ? (getMarketplaceDefaultShippingMethodsRes.data?.getMarketplaceDefaultShippingMethods?.length
                                        ? (
                                            <div className={classes.formField}>
                                                <div className={classes.divLabel}>
                                                    <span className={clsx(classes.label, 'capitalize')}>{cred.type.split('_').join(' ')}</span>
                                                </div>
                                                <Autocomplete
                                                    name="credentials.default_shipping_method"
                                                    mode="lazy"
                                                    className={classes.autocompleteRoot}
                                                    value={formik.values.credentials.default_shipping_method}
                                                    onChange={(e) => formik.setFieldValue('credentials.default_shipping_method', e)}
                                                    ...
                                                    primaryKey="value"
                                                    labelKey="label"
                                                    error={!!(formik.touched.credentials?.default_shipping_method
                                                        && formik.errors.credentials?.default_shipping_method)}
                                                    helperText={(formik.touched.credentials?.default_shipping_method
                                                        && formik.errors.credentials?.default_shipping_method) || ''}
                                                />
                                            </div>
                                        )
                                    ...
                                    : (
                                        <div className={classes.formField}>
                                            ...
                                            <TextField
                                                disabled={!cred.updatable}
                                                type={cred.type === 'cutoff_date' ? 'date' : 'string'}
                                                className={clsx(classes.fieldRoot, classes.fieldRootDesc)}
                                                variant="outlined"
                                                name={`credentials.${cred.type}`}
                                                value={formik.values.credentials[cred.type]}
                                                onChange={formik.handleChange}
                                                error={!!(formik.touched.credentials?.[cred.type] && formik.errors.credentials?.[cred.type])}
                                                helperText={(formik.touched.credentials?.[cred.type] && formik.errors.credentials?.[cred.type]) || ''}
                                                InputProps={{
                                                    className: classes.fieldInput,
                                                }}
                                            />
