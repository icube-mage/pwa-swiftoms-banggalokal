...
const ContentWrapper = (props) => {
    ...
    const [createThirdPartyApp] = gqlService.createThirdPartyApp();
    ...
    const handleSubmit = (input) => {
        ...
        createThirdPartyApp({
            ...
            .then(() => {
                ...
                window.toastMessage({
                    open: true,
                    text: t('thirdpartyapps:Third_Party_Apps_has_been_created'),
                    variant: 'success',
                });
    ...
    const modulesArr = () => {
        ...
        dataMod.forEach((mod) => {
            const tempMod = {
                ...mod,
                modules: mod.modules.map((mdl) => ({ ...mdl, status: false })),
            };
            if (mod.group.includes('webhook')) {
                mod.modules.forEach((mdl) => {
                    additional.push([mdl.module_code, false]);
    ...
    const formik = useFormik({
        ...
        validationSchema: Yup.object().shape({
            vendor_id: Yup.string().required(t('thirdpartyapps:This_is_a_Required_field')),
...
const Core = (props) => {
    ...
    React.useEffect(() => {
        BackdropLoad(aclCheckLoading || loadingMod);
    }, [aclCheckLoading, loadingMod]);

    if (aclCheckLoading || loadingMod) {
        return <Layout />;
    }
    ...
    if (!dataMod) {
        const errMsg = (errorMod?.message) ?? t('alluser:Data_not_found');
