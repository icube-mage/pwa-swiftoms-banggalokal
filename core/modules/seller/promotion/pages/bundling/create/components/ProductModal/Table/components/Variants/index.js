...
import useStyles from '@sellermodules/promotion/pages/bundling/create/components/ProductModal/Table/components/Variants/style';
...
const Variants = (props) => {
    ...
    const handleCheckedVariant = (value, variant) => {
        ...
        if (value) {
            const selectedVariant = [...temp[pIdx].selectedVariant, variant];
            temp[pIdx] = {
                ...temp[pIdx],
                selectedVariant,
            };
        } else {
            const selectedVariant = [...temp[pIdx].selectedVariant].filter((sv) => sv.entity_id !== variant.entity_id);
            temp[pIdx] = {
                ...temp[pIdx],
                selectedVariant,
            };
                                ...
                                <Grid item xs="auto">
                                    <Checkbox
                                        ...
                                        checked={!!checkedRows.find((checkedRow) => checkedRow.entity_id === row.entity_id)
                                            ?.selectedVariant?.find((sv) => sv.entity_id === variant.entity_id)}
                                    />
                                </Grid>
                                <Grid item xs="auto">
