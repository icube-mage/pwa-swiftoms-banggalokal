...
export const relativeTimeFrom = (date, t) => {
    ...
    dayjs.updateLocale('en', {
        relativeTime: {
            future: `${t('sellerchat:in')} %s`,
            past: `%s ${t('sellerchat:ago')}`,
            s: `${t('sellerchat:a_few_seconds')}`,
            m: `${t('sellerchat:a_minute')}`,
            mm: `%d ${t('sellerchat:minutes')}`,
            h: `${t('sellerchat:an_hour')}`,
            hh: `%d ${t('sellerchat:hours')}`,
            d: `${t('sellerchat:a_day')}`,
            dd: `%d ${t('sellerchat:days')}`,
            M: `${t('sellerchat:a_month')}`,
            MM: `%d ${t('sellerchat:months')}`,
            y: `${t('sellerchat:a_year')}`,
            yy: `%d ${t('sellerchat:years')}`,
        },
    });
    if (date) {
        ...
        sessionMessage.forEach((session) => {
            if (session.agent_code) {
                firebaseMessage.forEach((firebase) => {
                    if (`${session.customer_email}-${session.agent_code}` === firebase.chatId?.replaceAll(' ', '+')) {
                        const data = {
                            ...firebase,
