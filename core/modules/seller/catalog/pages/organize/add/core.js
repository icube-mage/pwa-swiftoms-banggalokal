...
const Core = (props) => {
    ...
    const [uploadProduct] = gqlService.uploadSellerProduct();
    ...
    const [downloadProductCsvTemplate] = gqlService.downloadProductCsvTemplate({
        onCompleted: (res) => {
            window.backdropLoader(false);
            router.push(res.downloadProductCsvTemplate);
        },
        onError: (e) => {
            window.backdropLoader(false);
            window.toastMessage({
    ...
    const [getActivity] = gqlService.getActivity({
        variables: {
            code: 'add_seller_product_at_once',
            by_session: true,
        },
        onCompleted: (res) => {
            window.backdropLoader(false);
    ...
    const formik = useFormik({
        ...
        onSubmit: (values) => {
            ...
            uploadProduct({ variables: { input: { binary, type: 'CREATE' } } })
                .then((res) => {
                    window.backdropLoader(false);
                    getActivity();
    ...
    const downloadTemplate = (types) => {
        window.backdropLoader(true);
        downloadProductCsvTemplate({ variables: { type: types } });
    };
