...
import useStylesParent from '@sellermodules/catalog/plugins/Variants/style';
import useStyles from '@sellermodules/catalog/plugins/Variants/Table/style';
                    ...
                    <Table className={classes.table}>
                        <TableHead>
                            <TableRow className={clsx(classes.tr, 'head')}>
                                ...
                                <TableCell className={classes.th}>{t('sellercatalog:Weight')}</TableCell>
                        ...
                        <TableBody>
                            <TableRow className={clsx(classes.rowItem, checked.length && 'selected')}>
                                <TableCell
                                    ...
                                    colSpan={formik.values.variants?.length + 3}
                                ...
                                <TableCell className={clsx(classes.td, checked.length && 'change', !checked.length && 'noborder')}>
                                    <Collapse in={!!checked.length} timeout="auto" unmountOnExit>
                                        <TextField
                                            ...
                                            value={field.price}
                                            onChange={(e) => setField((prev) => ({ ...prev, price: e.target.value.replace(numberReg.float, '') }))}
                                            disabled={!formik.values.sku}
                                        />
                                    </Collapse>
                                </TableCell>
                                <TableCell className={clsx(classes.td, checked.length && 'change', !checked.length && 'noborder')}>
                                    <Collapse in={!!checked.length} timeout="auto" unmountOnExit>
                                        <TextField
                                            className={clsx(classes.textInput, 'adorn', 'end')}
                                            InputProps={{
                                                endAdornment: (
                                            ...
                                            value={field.weight}
                                            onChange={(e) => setField((prev) => ({ ...prev, weight: e.target.value.replace(numberReg.float, '') }))}
                                            disabled={!formik.values.sku}
                                        />
                                    </Collapse>
                                </TableCell>
                            ...
                            {formik.values.variant_table?.map((item, i) => (
                                <TableRow key={i} className={clsx(classes.rowItem, i % 2 && 'dark', checked.includes(i) && 'selected')}>
                                    ...
                                    <TableCell className={classes.td}>
                                        <Switch
                                            value={item.status}
                                            onChange={() => formik.setFieldValue(`variant_table[${i}].status`, !item.status)}
                                            trueLabel=""
                                            falseLabel=""
                                            disabled={!formik.values.sku}
                                        />
                                    ...
                                    <TableCell className={classes.td}>
                                        <TextField
                                            ...
                                            onChange={(e) => formik.setFieldValue(`variant_table[${i}].price`,
                                                e.target.value.replace(numberReg.float, ''))}
                                            InputProps={{
                                                startAdornment: (
                                                    <InputAdornment className={classes.adornment} position="start">
                                                        {currency}
                                    ...
                                    <TableCell className={classes.td}>
                                        <TextField
                                            className={clsx(classes.textInput, 'adorn', 'end')}
                                            value={item.weight}
                                            onChange={(e) => formik.setFieldValue(`variant_table[${i}].weight`,
                                                e.target.value.replace(numberReg.float, ''))}
                                            InputProps={{
                                                endAdornment: (
                                                    <InputAdornment className={clsx(classes.adornment, 'end')} position="end">
                                            ...
                                            error={!!(formik.touched?.variant_table?.[i]?.weight
                                                && formik.errors?.variant_table?.[i]?.weight)}
                                            helperText={(formik.touched?.variant_table?.[i]?.weight
                                                && formik.errors?.variant_table?.[i]?.weight) || ''}
                                            disabled={!formik.values.sku}
                                        />
                                    </TableCell>
